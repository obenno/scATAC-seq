...
scATAC_obj$high.tss <- ifelse(scATAC_obj$TSS.enrichment > 2, 'High', 'Low')
tssPlot <- TSSPlot(scATAC_obj, group.by = 'high.tss') + NoLegend()

scATAC_obj$nucleosome_group <- ifelse(scATAC_obj$nucleosome_signal > 4, 'NS > 4', 'NS < 4')
...
tribble(
  ...
  "Raw Reads", "100",
  "Valid barcodes", "100%",
  "Q30 bases in barcodes", "100%",
  "Q30 bases in read 1", "100%",
  "Q30 bases in read 2", "100%"
) %>% kbl() %>% kable_paper("hover") %>%
  column_spec(1, bold = T)
```
...
```{r}
ggplot(d, aes(counts, color=type, fill=type)) + geom_density(alpha=0.4)+
...
tribble(
  ~"", ~"",
  "Uniquely mapped reads", "100",
  "Unmapped reads", "100",
  "Non-nuclear fragments", "100%",
  "Fragments in nucleosome-free regions", "100",
  "Fragments in mono-nucleosome regions", "100",
) %>% kbl() %>% kable_paper("hover") %>%
  column_spec(1, bold = T)
```
