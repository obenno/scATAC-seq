...
output:
  flexdashboard::flex_dashboard:
    ...
    theme:
      ...
      base_font: 'nudista-web, "Ropa Sans", Arial, sans-serif'
params:
  nCPUs: 6
  nMem: 10000000000.0
  sampleName: NULL
...
```{r include=FALSE}
html_font <- 'nudista-web, "Ropa Sans", Arial, sans-serif'
```
...
) %>% kbl() %>% kable_paper("hover", html_font = html_font) %>%
...
ggplot(d, aes(x=fragments, y=TSS.enrichment, color=group))+geom_point(alpha=0.8)+scale_x_log10() + 
    theme_bw() + scale_color_manual(values=c("grey", "steelblue")) +
    theme(legend.title=element_blank(), panel.grid=element_blank())
```
